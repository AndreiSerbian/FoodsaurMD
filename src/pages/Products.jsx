...
const Products = () => {
  ...
  // Create an array of images to display in the carousel
    ...
    <div className="min-h-screen pb-20">
      <div className="container mx-auto px-4 py-8">
        <motion.div 
          ...
          className="mb-8"
        >
          <Link to={`/category/${encodeURIComponent(producer.categoryName)}`} className="inline-flex items-center text-gray-600 hover:text-primary transition duration-200">
        ...
        <motion.div 
          ...
          className="mb-8 relative rounded-3xl overflow-hidden"
        >
          <Carousel className="w-full">
            <CarouselContent>
              {producerImages.map((image, index) => (
                <CarouselItem key={index} className="h-64">
                  <div className="h-full relative">
                    <img 
                      src={image.url} 
                      alt={`${producer.producerName} - ${image.label}`} 
